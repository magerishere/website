      ...
      @if ($category->name)
        <tr id="{{$category->id}}">
          ...
          <td class="categoryId">{{$category->id}}</td>
           <td  data-toggle="popover"  data-html="true" data-content="برای ویرایش کلیک کنید" class="categoryName"><input type="text" id="inputCategoryName{{$category->id}}" value="{{$category->name}}" disabled><div id="editButtonCategory"></div></td> {{-- onclick="createEditFormCategory({{$category->id}},'{{$category->name}}')" --}}
